...
/* Overview
   ...
   Keep filesystem requests in flight while the main thread parses the
   archive.  Creates, directory operations, data writes and closes use
   io_uring; operations with no asynchronous form (times, ownership and
   permissions) run synchronously on the main thread.  No user threads
   are created.

   The completion loop runs between members, after each data chunk, and
   whenever a barrier or the buffer budget requires progress.  The kernel
   continues requests already submitted while the parser reads the archive,
   but dependent requests wait until the parser runs the loop again.

   Parent-before-child, repeated-name and hard-link ordering are enforced
   from local state.  Aliased paths run in archive order.  Parent directory
   descriptors are resolved with RESOLVE_BENEATH and shared while in use.
   Owned data chunks remain stable until their write completions arrive.

   Directory metadata uses extract.c's delayed_set_stat machinery and is
   restored synchronously, deepest first, after all members have finished.
   Unsupported members use ordinary extraction after a barrier.  */
...
enum { DATA_BUDGET = 256 << 20 };	/* bytes awaiting write completion */
...
/* ------------------------------------------------------------------ */
/* Diagnostics matching the ordinary extraction path.  All callers run on
   the main thread, so ordinary quotearg slots need no synchronization.  */

static char const *
q0 (char const *name)
{
  return quotearg_n_style_colon (0, shell_escape_quoting_style, name);
}

static char const *
q1 (char const *name)
{
  return quotearg_n_style (1, shell_escape_quoting_style, name);
}
...
/* Where a job is in its asynchronous lifecycle.  */
...
struct job
{
  ...
  bool orphaned;		/* completion processing finished a job the
				   parser is still feeding */
  ...
  /* At most one write per file is outstanding.  Its buffer stays at
     CHUNK_HEAD until the entire chunk has completed.  */
...
/* Keep a completed job alive while the parser is still feeding it.
   Failed opens may finish before even the first data chunk is available.  */
static void
job_release (struct job *j)
{
  if (j->kind == JOB_FILE && ! j->data_done)
    {
      j->orphaned = true;
      while (j->chunk_head)
        {
...
/* ------------------------------------------------------------------ */
/* Intrusive ready-work queues, owned by the main thread.  */
...
static struct jobq in_q;
static struct jobq meta_q;
...
/* Includes parent-descriptor closes, not attached to member jobs.  */
static bool pending_operations (void);
...
parallel_barrier (void)
{
  if (! parallel_active)
    return;
  while (inflight || pending_operations ())
    pump (true);
  /* Namespace changes bypass the ordinary extractor's cache.  */
  fdbase_clear ();
}
...
/* Descriptor slots held by files and symlinks until their closes finish,
   plus the parent-directory cache.  Synchronous metadata needs at most
   two additional descriptors.  */
static idx_t slots_max, slots_used;
static idx_t pdir_max;

/* ------------------------------------------------------------------ */
/* Synchronous metadata, mirroring set_stat in extract.c.  */
...
/* ------------------------------------------------------------------ */
/* Special files have no asynchronous creation opcode.  */
...
/* ------------------------------------------------------------------ */
/* Asynchronous dependency tracking and request submission.  */
...
/* A failed submission or wait does not cancel requests or remove
   ...
   Do not let the fatal-exit hook drain a broken ring or free request
   storage that the kernel may still be using.  Process exit releases it.  */
...
/* Whether any member encountered a potentially aliased path.  */
...
/* Close FD asynchronously.  J, if not null, finishes at completion.  */
static void
...
/* Start or resume a file's next write.  The parser and completions both
   call this, but never recursively through the completion loop.  */
static void
write_file (struct job *j)
{
  if (j->orphaned || j->fd < 0 || j->writing)
    return;
  if (j->write_failed)
    while (j->chunk_head)
      {
  ...
  /* The parser can call here outside pump.  Allocation failure must not
     let its fatal-exit hook drain a partially constructed request.  */
  bool was_pumping = pumping;
  pumping = true;
  idx_t i = op_alloc (OP_WRITE, j);
  struct io_uring_sqe *sqe = get_sqe ();
  io_uring_prep_write (sqe, j->fd, c->data + j->chunk_written,
                      c->len - j->chunk_written, j->written);
  io_uring_sqe_set_data64 (sqe, i);
  j->writing = true;
  pumping = was_pumping;
}
...
{
  /* Callbacks may allocate operations and move the operation table.  */
  struct op saved = *op_take (io_uring_cqe_get_data64 (cqe));
  struct op const *op = &saved;
  int res = cqe->res;
  struct job *j = op->job;
    ...
    case OP_WRITE:
      {
        ...
        if (res <= 0)
          {
            errno = res < 0 ? -res : 0;
            write_error_p (j->name, j->chunk_written, c->len);
            j->write_failed = true;
          }
        else
          {
        ...
        if (j->write_failed || j->chunk_written == c->len)
          {
            j->chunk_head = c->next;
            if (! j->chunk_head)
              j->chunk_tail = NULL;
...
ring_setup (void)
{
     ...
     refuse those so that extraction falls back to the ordinary path.  */
  {
    static int const needed[] = { IORING_OP_OPENAT2, IORING_OP_MKDIRAT,
				  ...
				  IORING_OP_CLOSE, IORING_OP_WRITE };
...
static bool
pending_operations (void)
{
  return ops_inflight != 0;
}

/* Retry resource shortages without mistaking a no-op for progress.  */
static void
flush_ring (void)
{
  while (io_uring_sq_ready (&ring))
    {
      ...
      check_io_result (r == 0 && io_uring_sq_ready (&ring) ? -EAGAIN : r,
                       _("io_uring submission failed"));
    }
}

/* One completion-loop turn.  Never called from a completion callback:
   callbacks only append ready work or SQEs.  Flush SQEs before potentially
   slow metadata calls and again before returning to the archive parser.  */
static void
pump (bool wait)
{
  pumping = true;
  struct job *j;
  while (in_q.head && ops_inflight < MAX_INFLIGHT_OPS)
    handle_job (jobq_pop (&in_q));
  flush_ring ();

  /* Detach the current batch: finishing a special file may release more
     jobs, whose metadata belongs to the next completion-loop turn.  */
  struct jobq ready = meta_q;
  meta_q.head = meta_q.tail = NULL;
  while ((j = jobq_pop (&ready)))
    {
      if (j->kind == JOB_NODE || j->kind == JOB_FIFO)
        {
          make_special (j);
          finish_job (j);
        }
  ...
  if (wait && ops_inflight)
    {
      struct io_uring_cqe *cqe;
      check_io_result (io_uring_wait_cqe (&ring, &cqe),
                       _("io_uring wait failed"));
    }

  /* Consume each CQE before its callback can submit more work.  Bound
     the batch so a stream of completions cannot starve archive parsing.  */
  unsigned count = io_uring_cq_ready (&ring);
  for (unsigned i = 0; i < count; i++)
    {
      struct io_uring_cqe *cqe;
      int r = io_uring_peek_cqe (&ring, &cqe);
      if (r < 0)
        {
          check_io_result (r == -EAGAIN ? 0 : r, _("io_uring wait failed"));
          break;
        }
      struct io_uring_cqe saved = *cqe;
...
parallel_init (void)
{
  ...
  /* Reserve descriptors for ordinary extraction, the ring, the working
     directory and synchronous metadata; divide the rest between the
     parent cache and in-flight files.  */
  struct rlimit rl;
  ...
  /* Limit owned data buffers under a finite address-space/data limit.  */
  {
...
/* A fatal archive read must finish the file with the data already read.  */
...
parallel_extract_member (char *file_name, char typeflag)
{
  ...
  /* As in set_stat, warn about implausible archive timestamps.  */
  ...
  if (kind == JOB_FILE && j->size > 0)
    {
      /* J stays valid until the parser sets data_done, even if its
         asynchronous open fails while streaming.  */
      off_t size = j->size;
      struct chunk *c = NULL;
      while (size > 0)
	{
...
/* Drain all writes, metadata and closes before restoring directories.  */
void
parallel_finish (void)
{
  /* Allocation failures in completion callbacks reach the fatal hook.
     Do not recursively enter a half-completed callback or free storage
     referenced by outstanding kernel requests.  */
  if (pumping)
    parallel_active = false;
  if (! parallel_active)
    return;
...
/* ------------------------------------------------------------------ */
/* Restore directory metadata synchronously, deepest first so restrictive
   parent modes never prevent access to children.  */
...
parallel_apply_dirstats (struct parallel_dirstat *arr, idx_t n)
{
  ...
  for (idx_t i = 0; i < n; i++)
    {
      arr[i].wdfd = wd_fd_for (arr[i].change_dir);
      dirstat_apply (&arr[i]);
    }
}
