...
/* Normalize NAME for map lookups: drop "./" prefixes, empty and "."
   components, and a trailing slash.  A leading slash (possible only
   with --absolute-names) is kept, so that parent paths derived from
   the key are absolute too.  Names have already been made safe by
   safer_name_suffix, so ".." is left alone.  */
static char *
...
{
  return ((absolute_names_option ? 0 : RESOLVE_BENEATH)
...
parallel_extract_member (char *file_name, char typeflag)
{
  ...
  switch (typeflag)
    ...
    case SYMTYPE:
      ...
      break;
    case LNKTYPE:
      if (absolute_names_option && contains_dot_dot (st->link_name))
	return ordinary_member ();	/* placeholder logic in extract_link */
      kind = JOB_LINK;
      break;
  ...
  /* With --absolute-names, ".." can spell one directory two ways,
     which ordering by name cannot see; such members go the ordinary
     way.  Without it, safer_name_suffix has already removed "..".  */
  if (absolute_names_option
      && (contains_dot_dot (file_name)
	  || (kind == JOB_LINK && contains_dot_dot (st->link_name))))
    return ordinary_member ();
  ...
  if (! *normalized || streq (normalized, "/"))
    {
      /* The directory we extract into ("." or "/"): only metadata.  Let
	 extract_dir record it, once the pipeline is idle.  */
      free (normalized);
      return ordinary_member ();
    }
...
/* Different -C contexts can name the same directory.  Keep its most
   recent metadata regardless of the order of the final depth sort.  */
...
parallel_apply_dirstats (struct parallel_dirstat *arr, idx_t n)
{
  if (n == 0)
    return;
  /* One name per directory unless the run saw aliases or used several
     destinations, which may overlap; then the newest metadata wins.  */
