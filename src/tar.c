...
parse_opt (int key, char *arg, struct argp_state *state)
{
  ...
  switch (key)
    ...
    case PARALLEL_MAX_META_THREADS_OPTION:
      {
	char *end;
	idx_t n = stoint (arg, &end, NULL, 1, IDX_MAX);
	if (*end || n == 0)
