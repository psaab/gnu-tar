...
This branch provides two engines selected at configure time. The default,
also selected by `--without-pthreads`, drives io_uring from the main
thread and applies metadata operations synchronously. It creates no
application worker threads. The kernel can still use io-wq workers to
service asynchronous requests. Configure with `--with-pthreads` to use
the alternative engine with I/O, writer, and adaptive metadata workers.
...
Both engines need liburing. `--without-liburing` disables both engines;
explicit `--with-pthreads` fails at configure time if liburing or pthreads
is unavailable, including when combined with `--without-liburing`.
The metadata-pool options `--parallel-meta-threads` and
`--parallel-max-meta-threads` are visible and active only in threaded
builds. Thread-free builds hide them, validate their arguments, and warn
that valid settings are ignored. Without either engine, all parallel
options are hidden; requests to enable or tune it warn and remain
sequential.

## Historical measurement

These measurements predate the configure-time engine switch. They have
not been rerun for the current build-selection changes; the binary hashes
below identify the historical executables.
...
Both historical default-mode runs issued 5,201 timestamp syscalls;
`--touch` issued none. The first synthetic threaded run made 137 `clone3`
calls as its metadata pool expanded. The prototype made no `clone` or
`clone3` calls and initialized one io_uring instance. Capping the old
metadata pool at one worker with
`--parallel-meta-threads=1 --parallel-max-meta-threads=1` took 26.66 seconds,
confirming that concurrent timestamp calls account for the threaded
...
## Building and testing both engines

Use a bootstrapped source tree at one revision that has not been
configured in place. For a Git checkout, follow `README-hacking` to
generate `configure` first. Keep the two build directories separate:

```sh
tar_src=/absolute/path/to/unconfigured/gnu-tar
tar_builds=`mktemp -d "${TMPDIR:-/tmp}/tar-engine-builds.XXXXXX"`
mkdir "$tar_builds/threaded" "$tar_builds/nothreads"

(cd "$tar_builds/threaded" &&
 "$tar_src/configure" --with-liburing --with-pthreads &&
 make -j8 && make check TESTSUITEFLAGS='-j8')

(cd "$tar_builds/nothreads" &&
 "$tar_src/configure" --with-liburing --without-pthreads &&
 make -j8 && make check TESTSUITEFLAGS='-j8')
```

Replace `--with-liburing` with `--with-liburing=/path/to/prefix` in both
commands if liburing is installed outside the compiler's search paths.
On a system that cannot use io_uring, engine-dependent tests may skip;
passing the remaining tests does not validate asynchronous extraction.
The benchmark's explicit parallel preflight below must also succeed.

To check option compatibility and ordinary extraction without either
engine, use a third build directory:

```sh
mkdir "$tar_builds/noengine"
(cd "$tar_builds/noengine" &&
 "$tar_src/configure" --without-liburing --without-pthreads &&
 make -j8 && make check TESTSUITEFLAGS='-j8')
```

## Comparing the builds

Both benchmark executables can now come from the same source revision;
there is no need to switch branches or replace a previous executable:

```sh
"$tar_src/scripts/benchmark-parallel-extract" \
  "$tar_builds/threaded/src/tar" "$tar_builds/nothreads/src/tar"
"$tar_src/scripts/benchmark-parallel-extract" --synthetic-delay=5ms \
  "$tar_builds/threaded/src/tar" "$tar_builds/nothreads/src/tar"
```
